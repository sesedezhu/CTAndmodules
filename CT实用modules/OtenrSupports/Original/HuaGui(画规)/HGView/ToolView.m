...
#define kButtonSpace CTStopStatusRect/2
//深绿色
...
-(void)createButtonsWithArray:(NSArray *)array
{
    ...
    for(NSString *text in array){
        ...
        button.titleLabel.font = [UIFont systemFontOfSize: CONVER_VALUE(18.0f )];
...
-(void)forceHideView:(UIView *)compareView
{
    ...
    toFrame.origin.y = -(CTStopStatusRect+CTStopNavRect);
    toolFrame.size.height = (CTStopStatusRect+CTStopNavRect);
...
-(void)showHideView:(UIView *)view
{
    ...
    if (toFrame.origin.y < 0) {
        //隐藏时显示
        toFrame.origin.y = (CTStopNavRect+CTStopStatusRect-6);  //控制颜色高距离,红线请去MyButton修改
        toolFrame.size.height = (CTStopNavRect+CTStopStatusRect+24);
    }else{
        toFrame.origin.y = -(CTStopStatusRect+CTStopNavRect);
        toolFrame.size.height = (CTStopStatusRect+CTStopNavRect);
    }
